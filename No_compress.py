...
# --- FUNGSI DEKOMPRESI YANG DIMODIFIKASI ---
def decompress_payload_from_raspi(compressed_parts, metadata):
    # Jika metadata menunjukkan bahwa ini adalah data mentah (tanpa DWT/Quant/Zlib)
    # kita asumsikan hanya ada satu "part" dan itu adalah data mentah float32.
    if len(metadata) == 1 and metadata[0]["q_step"] == 1.0 and metadata[0]["compressed_size"] == metadata[0]["length"] * 4: # Asumsi float32 = 4 bytes/sample
        # Ini adalah data mentah (setelah filter) yang dikirim langsung
        raw_bytes = compressed_parts[0]
        ecg_signal = np.frombuffer(raw_bytes, dtype=np.float32)
        return ecg_signal
    else:
        # Ini adalah data terkompresi DWT + Quant + Zlib (mode lama)
        coeffs = []
...
def process_payload(payload, device_id):
    ...
    compressed_or_raw = payload[:sep_idx] # Nama variabel diubah
    metadata_json = payload[sep_idx + len(sep_marker):].decode()
    metadata = json.loads(metadata_json)

    # Kita perlu memecah compressed_or_raw menjadi parts berdasarkan metadata
    # Ini penting karena fungsi decompress_payload_from_raspi mengharapkan list of bytes
    parts = []
    offset = 0
    for meta in metadata:
        part_len = meta["compressed_size"]
        parts.append(compressed_or_raw[offset:offset + part_len])
        offset += part_len

    try:
        # Panggil fungsi dekompresi yang dimodifikasi
        ecg_signal = decompress_payload_from_raspi(parts, metadata)
        # Filter tetap dilakukan di gateway untuk konsistensi
    ...
    try:
        ...
        if result:
            ...
            for val in ecg_signal: # Simpan ecg_signal yang sudah didekompresi/mentah
